...
import { buildPredictionPrompt } from "../libs/predictionPrompt.js";
import { encodeShort, decodeShort } from "../utils/cryptoUtil.js";
...
class AuthService {
  ...
  async lineUIDCheck(lineUid, token) {
    ...
    const checkUser = await this.authRepo.getUserByLineUidAndRestaurant(
      lineUid,
      restaurantId,
    );
  ...
  async register(data, token) {
    ...
    const checkMember = await this.authRepo.getUserByLineUidAndRestaurant(
      data.lineUid,
      restaurantId,
    );
  ...
  async editProfile(userId, data) {
    ...
    if (birthChanged) {
      const BaziURL = process.env.BAZI_URL;
      const [year, month, day] = (data.birth_date ?? user.birth_date)
        .split("-")
        .map(Number);
      const [hour, minute] = (data.birth_time ?? user.birth_time)
        .split(":")
        .map(Number);
  ...
  async prediction(userId) {
    ...
    const existingPrediction = await this.authRepo.getPredictionByUserAndDate(
      userId,
      dbDate,
    );
          ...
          messages: [
            {
              role: "system",
              content:
                "คุณเป็นนักเขียนคอลัมน์ดวงรายวันภาษาไทย สำนวนธรรมชาติ เหมือนคนเขียนจริง ไม่ใช้สำนวนระบบหรือบันทึกส่วนตัว",
            },
  ...
  async findMenu(userId, page) {
    ...
    const rows = await this.authRepo.getAllMenuElementLikeRows(
      restaurantId[0].restaurant_id,
      userId,
    );
    const lastPage = Math.ceil(rows[0].total / limit);
