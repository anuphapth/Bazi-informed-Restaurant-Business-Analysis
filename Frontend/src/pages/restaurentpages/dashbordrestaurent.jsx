...
function DashboardRestaurant() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        ...
        await axios.put(`${API_URL}/api/restaurant/edit/menu`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        })
      } else {
        await axios.post(`${API_URL}/api/restaurant/add/menu`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        })
      }
      closeModal()
  ...
  const confirmDeleteMenu = async () => {
    ...
    try {
      setDeleting(true)
      await axios.delete(`${API_URL}/api/restaurant/delete/menu`, {
        headers: { Authorization: `Bearer ${token}` },
...
const openAddModal = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FCFBFA] text-slate-900 font-line antialiased">

      {/* HEADER SECTION */}
      <div className="relative bg-red-950 pt-12 pb-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-red-200/70 mt-2 font-medium">จัดการเมนูและธาตุอาหารสำหรับลูกค้าของคุณ</p>
            </div>

            <button
              onClick={openAddModal}
              className="group flex items-center gap-3 bg-amber-500 hover:bg-amber-400 text-red-950 px-8 py-4 rounded-2xl font-black transition-all shadow-xl shadow-amber-500/20 active:scale-95 self-start md:self-center"
            >
      ...
      <main className="max-w-7xl mx-auto px-6 -mt-12 relative z-20 pb-20">
        ...
        {!loading && menus.length > 0 && (
          <div className="flex justify-end mb-6">
            <div className="bg-white/80 backdrop-blur-md p-1.5 rounded-2xl shadow-xl shadow-red-900/5 border border-amber-100 flex gap-1">
              <button
                onClick={() => setViewMode("grid")}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all font-black text-xs uppercase tracking-widest ${viewMode === 'grid' ? 'bg-red-900 text-amber-400' : 'text-slate-400 hover:bg-slate-50'}`}
              >
                <LayoutGrid size={16} /> <span className="hidden sm:inline">Grid</span>
              </button>
              <button
                onClick={() => setViewMode("list")}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all font-black text-xs uppercase tracking-widest ${viewMode === 'list' ? 'bg-red-900 text-amber-400' : 'text-slate-400 hover:bg-slate-50'}`}
              >
                ...
                viewMode === 'grid' ? (
                  ...
                  <div key={menu.id} className="group bg-white rounded-[2.5rem] p-4 shadow-xl shadow-red-950/5 hover:shadow-2xl hover:shadow-red-900/10 transition-all duration-500 flex flex-col border border-amber-50/50">
                    <div className="relative aspect-square rounded-[2rem] overflow-hidden bg-slate-100 shadow-inner">
                      <img
                        src={menu.image_url ? (menu.image_url.startsWith("http") ? menu.image_url : `${API_URL}${menu.image_url}`) : "https://via.placeholder.com/400x400"}
